...
const projects = [
  { id: "01", type: "PROJECT · GENOMIC DATA", name: "Avian Influenza Dashboard", tech: "React · Node.js · Express · PostgreSQL", summary: "A full-stack web application for analyzing and visualizing over 10,000 genomic datasets, with interactive React components and backend services for data visualization.", detail: "Designed data cards and frontend interactions to communicate complex datasets clearly and effectively.", image: "/project-avian.svg", liveUrl: "https://mutanalyzer.tiiny.site/" },
  { id: "02", type: "PROJECT · SEQUENCE PROCESSING", name: "VNTRseeker", tech: "Node.js · HTML · CSS · JavaScript", summary: "A web application for efficient bulk-sequence data processing with optimized backend logic for repeat-sequence workflows.", detail: "Implemented optimized backend logic resulting in a 25% improvement in data pipeline efficiency.", image: "/project-vntr.svg", liveUrl: "https://vntrseeker.vercel.app/" },
  { id: "03", type: "PROJECT · TRAVEL PLATFORM", name: "Infi Yatra", tech: "React · Node.js · Express.js", summary: "A responsive travel web application with dynamic routing and API integrations for a seamless booking experience.", detail: "Developed the frontend and backend experience for browsing travel options and moving through a streamlined booking flow.", image: "/project-infi-yatra.svg", liveUrl: "https://infi-yatra.vercel.app/" },
];
...
export default function Home() {
  ...
  return <div className={`editorial-site ${loaded ? "is-ready" : ""}`}>
    ...
    <main id="top">
      ...
      <section id="work" className="page-section numbered-section work-section"><div className="section-label mono">[ 02 ] SELECTED WORK</div><div className="section-intro"><h2>Projects that move information from <em>raw to readable.</em></h2><p>Selected systems from Yash’s software and bioinformatics work. Visuals are decorative representations; project facts come from the supplied résumé.</p></div><div className="project-rows">{projects.map((project) => <article key={project.id} className="project-row"><div className="project-copy"><span className="mono caps">{project.type}</span><h3>{project.name}</h3><p>{project.summary}</p><div className="project-actions"><button className="text-link" onClick={() => setSelected(project)}>See project details <ArrowUpRight size={14} /></button><a className="text-link live-link" href={project.liveUrl} target="_blank" rel="noreferrer">Open live project <ArrowUpRight size={14} /></a></div></div><div className="project-visual"><img src={project.image} alt="" /><span className="visual-label mono">{project.tech}</span></div></article>)}</div></section>
